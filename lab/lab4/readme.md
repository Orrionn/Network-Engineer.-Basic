...
> На интерфейс G0/0 назначены 2 многоадрессные рассылки.
 ...
 FF02::1:FF – это мультикастоый адрес, на который автоматически подписываются все узлы. После неизменной части FF02::1:FF оставшиеся 24 бита берутся с ip которые настроены на интерфейсе. Взята она со следующего ip: 2001:DB8:ACAD:A::1(2001:DB8:ACAD:A:0000:0000:0000:1000). 
               FF02:0000:0000:0000:0000:1000:FF00:1000

### Шаг 2. Активируйте IPv6-маршрутизацию на R1.
#### a.	В командной строке на PC-B введите команду ipconfig, чтобы получить данные IPv6-адреса, назначенного интерфейсу ПК.

>#### Назначен ли индивидуальный IPv6-адрес сетевой интерфейсной карте (NIC) на PC-B?
> Нет, не назначен. Назначен только link-local адрес, который генерируется автоматически механизмом EUI-64.
...
>Локальный адреса канала, используется для взаимодействия IPv6- устройств между собой, но только внутри локальной сети. А т.к. оба интерфейса находятся в разных сегментах сети, то узлы этих сетей не будут пересекаться. И будут обращены конкретному интерфейсу, либо G0/0, либо G0/1
